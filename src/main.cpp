...
int main(int, char **) {
  ...
  //  auto www = c_unique_ptr<GLFWwindow, glfwDestroyWindow(window)> {
  ...
  auto font_cousine = io.Fonts->AddFontFromFileTTF(
      CPP_SRC_DIR "modules/imgui/misc/fonts/Cousine-Regular.ttf", 15.0f);
  auto font_karla = io.Fonts->AddFontFromFileTTF(
      CPP_SRC_DIR "modules/imgui/misc/fonts/Karla-Regular.ttf", 18.0f);
  auto font_lato =
      io.Fonts->AddFontFromFileTTF(CPP_SRC_DIR "fonts/Lato-Regular.ttf", 18.0f);
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    if (show_theme_window) {
      ImGui::Begin("Themes", &show_theme_window);
      {
        ...
        ImGui::Combo("theme-combo", &theme_current, themes,
                     IM_ARRAYSIZE(themes));
